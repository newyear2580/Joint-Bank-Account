...
describe("BankAccount", function () {
  async function deployBankAccount() {
    ...
    // addr0, addr1, addr2, addr3, addr4 為 signer object
  ...
  describe( "Creating an account", () => {
    ...
    it ( "should not allow a user have more than 3 accounts", async () => {
      const { bankAccount, addr0 } = await loadFixture( deployBankAccount ) ;
      for ( let i = 0 ; i < 3 ; i ++ ) {
        await bankAccount.connect( addr0 ).createAccount( [] ) ;
      } // for
  ...
  describe( "Depositing", () => {
    it ( "should allow deposit from account owner", async () => {
      ...
      // bankAccount 這個智能合約擁有的 balance 增加 100
      // addr0 的帳戶 balance 減少 100
    } ) ;
  ...
  describe( "Withdraw", () => {
    ...
    describe( "Make withdraw", () => {
      ...
      it ( "should not allow creator of request to withdraw approved request twice", async () => {
        ...
        await expect( bankAccount.connect( addr0 ).withdraw( 0, 0 ) )
          .to.be.reverted ;
        // 不是因為不足夠的 balance 導致失敗，而是因為已經用這個 approved request 取款過
        // 不能利用這個 approved request 再做一次取款動作
      } ) ;

      it ( "should not allow non-creator of request to withdraw approved request", async () => {
        const { bankAccount, addr1 } = await deployBankAccountWithAccounts( 2, 100, [100] ) ;
        await bankAccount.connect( addr1 ).approveWithdraw( 0, 0 ) ;
        await expect( bankAccount.connect( addr1 ).withdraw( 0, 0 ) )
          .to.be.reverted ;
      } ) ;

      it ( "should not allow creator of request to withdraw approved request", async () => {
        const { bankAccount, addr0 } = await deployBankAccountWithAccounts( 2, 100, [100] ) ;
        await expect( bankAccount.connect( addr0 ).withdraw( 0, 0 ) )
          .to.be.reverted ;
      } ) ;
